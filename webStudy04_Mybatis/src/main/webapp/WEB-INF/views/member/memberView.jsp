...
<c:set var="mutable" value="false"></c:set> <!-- boolean mutable = false; -->
<c:if test="${not empty sessionScope.authMember and 'ROLE_ADMIN' ne sessionScope.authMember.mem_auth }"> <!-- if(authMember!=null && !"ROLE_ADMIN".equals(authMember.getMem_auth())) -->
	<c:if test="${sessionScope.authMember.mem_id eq member.mem_id }"> <!-- if(authMember.getMem_id().equals(member.getMem_id())) -->
		<c:set var="mutable" value="true"></c:set> <!-- mutable = true; -->
	</c:if>
</c:if>
<c:if test="${mutable }">
